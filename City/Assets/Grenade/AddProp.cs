...
public class AddProp : MonoBehaviour {
	 ...
	 * if necessary.
	 */
    void  OnTriggerEnter1 ( Collider collision  ){
        print("Collision");
        PlaySounds2.StartMusic = true;
    //private void OnCollisionEnter(Collision collision) {
		if (targetBone.IsChildOf(collision.transform)){
			...
			if(!checkProp){  
                ...
                collision.GetComponent<BasicController2>().HasRifle = true;
                //collision.GetComponent<BasicController2>().Rifle = ;
            }
		}
	}
    ...
    void OnTriggerEnter(Collider collision) {
        ...
        if (targetBone.IsChildOf(collision.transform)) {
            ...
            if (!checkProp) {
                // IF prop is not in place yet, THEN create a new game object...
                GameObject newprop = Instantiate(prop, targetBone.position, targetBone.rotation) as GameObject;
                ...
                Player.AddWeapon(PropName, newprop);

                newprop.transform.parent = targetBone;
                newprop.transform.localPosition += Player.Weapon.PositionOffset;
                newprop.transform.localEulerAngles += Player.Weapon.RotationOffset;
