...
public class TextureFromCamera : MonoBehaviour
{
    ...
    private int imgNum { get { return Directory.GetFiles(Application.dataPath + "/../Captured/").Length; } }

    void Start()
    {
        if (rawImagePhoto == null)
        {
            rawImagePhoto = GameObject.FindGameObjectWithTag("RawImg");
        }
        if (imageFrame == null)
        {
            imageFrame = GameObject.FindGameObjectWithTag("ImgFrame");
        }
    }
    ...
    IEnumerator CaptureScreen()
    {
        ...
        File.WriteAllBytes(Application.dataPath + "/../Captured/"+imgNum+".png", bytes);
    }
}